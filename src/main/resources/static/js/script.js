...
app.controller('monopolyController', function($scope) {
	...
	$scope.chooseToken = function() {
		$joinName.hide();
		$scope.state.players.forEach(function(player, i, players) {
            if(player.name == $username){
                $scope.join(player.token.type);
            }
         });
		$joinToken.show();
	}
	...
	$scope.quit = function() {
		$scope.getOp('quit?name=' + $scope.username);
	}
	...
	$scope.cardStyle = function (tile) {
		...
		if (tile.type == 'UTILITY'){
			...
			if (tile.name == "WATER WORKS") {
				imageLocation = "/static/img/waterworks.png";
			} else {
				imageLocation = "/static/img/electricompany.png";
			}
	...
	$scope.tokenStyle = function(player) {
		...
		var style = {
			'background-image': 'url("/static/img/' + player.token.type.toLowerCase() + '.png")',
			left: position.left,
			top: position.top,
...
function rolldiceaux($scope) {
	var dice1 = document.getElementById('dice-one');
	var dice2 = document.getElementById('dice-two');

	var backgrounds = [
	"url('/static/img/dice1.png')",
	"url('/static/img/dice2.png')",
	"url('/static/img/dice3.png')",
	"url('/static/img/dice4.png')",
	"url('/static/img/dice5.png')",
	"url('/static/img/dice6.png')"];
	...
	if (counter > 10 || $scope.state.phase != "ROLLING"){
		dice1.style.backgroundImage = backgrounds[$scope.state.board.dice[0].value - 1];
		dice2.style.backgroundImage = backgrounds[$scope.state.board.dice[1].value - 1];
		return;
	}
	else{
		dice1.style.backgroundImage = backgrounds[Math.floor((Math.random() * 6) + 1)];
		dice2.style.backgroundImage = backgrounds[Math.floor((Math.random() * 6) + 1)];
	}
